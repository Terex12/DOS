...
trait Service extends HttpService with Json4sSupport {
  ...
  val RSA = actorRefFactory.actorOf(Props[Cache], "Cache")
  val AES = actorRefFactory.actorOf(Props[Key], "Key")
      ...
      path(Segments) { segments =>
        validate(segments.exists(_.toString.matches("[0-9]*")), "unmatched path"){
          parameterSeq{ params =>
            ...
            ctx => op ! GetRequest(path, para, ctx, mmu, RSA, AES, rsa)
          }
        }~
          ...
          validate(segments.exists(_.toString.matches("preregister")), "unmatched path"){
            parameterSeq{ params =>
              ...
              val para = params
              ctx => op ! CreateEmptyUser(ctx, mmu, RSA, AES, para, rsa)
            }
          }
      }
    }~
        ...
        path(Segments) { segments =>
          validate(segments.exists(_.toString.matches("page")), "unmatched path"){
            parameterSeq{ params =>
              ...
              val op: ActorRef = actorRefFactory.actorOf(Props[Operator])
              ctx => op ! CreateUser(path, para, ctx, mmu, RSA, AES, rsa)
            }
          }~
            validate(segments.exists(_.toString.matches("[0-9]*")), "unmatched path"){
              parameterSeq{ params =>
                ...
                val op: ActorRef = actorRefFactory.actorOf(Props[Operator])
                ctx => op ! PostRequest(path, para, ctx, mmu, RSA, AES, rsa)
              }
            }
        }
      }
