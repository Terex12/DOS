...
//    }
    ...
    case GetRequest(path, para, ctx, mmu, cache, key, rsa) =>{
      ...
      futureCurrentInfo.onSuccess{
        case (currentId:String, k: BigInt, module:BigInt) => {
                ...
                if(haveRight){
                  ...
                  futureInfo.onSuccess{
                    ...
                    case info: List[String] => {
                      ...
                      info.foreach(item => {
                        //RSA Encryption
                        s ++= rsa.encrypt(item, k, module).toString
                        s += ','
                      })

                      val future = key ? getKey(nodeId)  //get AES key and IV
                      val re = Await.result(future, timeout.duration).asInstanceOf[(String,String)]
                      val r1 = rsa.encrypt(re._1, k, module).toString
                      val r2 = rsa.encrypt(re._2, k, module).toString
                      val result = s.append(r1 + "," + r2).toString
                  ...
                  futureInfo.onFailure{
                    case error=>{
                      ...
                      val entity = HttpEntity(rsa.encrypt(nodeId, k , module).toString)
    ...
    case PostRequest(path, para, ctx, mmu, cache, key, rsa) =>{
...
//          else{                         //create new node
        ...
        //store (nodeId, aes, iv)
        var aes = rsa.decrypt(BigInt(para(para.size-3)._2), rsa.privateKey, rsa.modulus)
        var iv = rsa.decrypt(BigInt(para(para.size-2)._2), rsa.privateKey, rsa.modulus)
        
        println("In POSt SERver == Store KSY  :" + result._1 + " %% " +aes + " %% " +iv)
        key ! StoreKey(result._1, aes, iv)
    ...
    case CreateUser(path, para, ctx, mmu, cache, key, rsa) =>{
      //update user node
      println("~~~~Create User~~~~~~~~")
      ...
      if (flag == true){
        ...
        println("In POSt SERver == Store KSY  :" + currentId + " %% " +aes + " %% " +iv)
        println("In POSt SERver == Store KSY  :" + currentId + " %% " +aes + " %% " +iv)
        key ! StoreKey(currentId, aes, iv)
    ...
    case CreateEmptyUser(ctx, mmu, cache, key, para, rsa) =>{
